...
chk("rows carry pipeline", d["rows"][0]["pipeline"] in ("himalaya","competitor"), True)
...
p = c.get("/api/engine/results?channel=zepto&pipeline=himalaya&limit=5").json()
chk("pipeline filter works", all(x["pipeline"]=="himalaya" for x in p["rows"]), True)
