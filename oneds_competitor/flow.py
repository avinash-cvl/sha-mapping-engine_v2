...
# ============================================================
# FIX: Add project root to sys.path so sh_sdk is found
# ============================================================
import sys
import os
...
from oneds_competitor.stages import stage_ingest
from oneds_competitor.stages import stage_lexicon
...
from sha_observability_sdk import (
    ...
    capture_generation_response, confidence_score, message_feedback,
    get_current_generation
)

# Keep decorator alias as `obs` so existing decorators (@obs.observe) continue to work
obs = observe
...
@obs.observe(as_type="span", name="run_ingest")
def run_ingest(
...
@obs.observe(as_type="span", name="run_deterministic")
...
@obs.observe(as_type="span", name="candidates")
def run_candidates(
    run_id: str,
    conn: db.Connection,
    sources: list[SourceProduct],
...
@obs.observe(as_type="span", name="scoring")
def run_scoring(
    run_id: str,
    sources: list[SourceProduct],
    candidates_by_sku: dict[str, dict[str, tuple[float, float]]],
...
@obs.observe(as_type="span", name="disposition")
def run_disposition(
    run_id: str,
    sources: list[SourceProduct],
...
def _mapping_status(result: MatchResult) -> str:
    ...
    print(f"DEBUG: final_score={final_score}, ensemble_score={ensemble_score}")
...
def _match_source_in_worker(
    ...
    """Run one SKU with a worker-owned read connection."""
...
def run_matching_flow(
    ...
    try:
        ...
        # Workers match SKUs concurrently. The main thread saves each
        # completed future immediately, keeping checkpoint and DB writes safe.
