# Prompt — 1DS "vitamins & supplements" -> HGML
...
You receive marketplace product listings from the 1DS category **vitamins & supplements**.
...
1. **Himalaya product name.** If the title names a Himalaya product, use the
   ...
   - "amalaki" -> AMALAKI
   - "amalaki capsules" -> AMALAKI
   - "amalaki drink" -> AMALAKI
   - "amalaki tablets" -> AMALAKI
   - "arjuna" -> ARJUNA
   - "arjuna capsules" -> ARJUNA
...
**Note.** This category has a hand-audited bespoke script, `categories/vitamins_supplements.py`, which is more accurate than these rules alone. Use this prompt for the LLM route; use the script for the deterministic route.
