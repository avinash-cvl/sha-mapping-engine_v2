...
You receive marketplace product listings from the 1DS category **pet care**.
Assign each to exactly one HGML category / sub-category pair from the ALLOWED
LIST. The list is exhaustive and authoritative.

## Allowed targets

- `COMPANION CARE - FOODS` / `HEALTHY PET FOOD`
- `COMPANION CARE - FOODS` / `HEALTHY TREATS`
- `COMPANION CARE - GROOMING` / `ERINA`
- `COMPANION CARE - GROOMING` / `FRESH COAT`
- `COMPANION CARE - SUPPLEMENTS` / `ANXOCARE`
- `COMPANION CARE - SUPPLEMENTS` / `CANISEP`
- `COMPANION CARE - SUPPLEMENTS` / `DIGYTON`
- `COMPANION CARE - SUPPLEMENTS` / `FURGLOW`
- `COMPANION CARE - SUPPLEMENTS` / `HIMPYRIN`
- `COMPANION CARE - SUPPLEMENTS` / `IMMUNOL`
- `COMPANION CARE - SUPPLEMENTS` / `LIV.52`
- `COMPANION CARE - SUPPLEMENTS` / `MOBILITY PLUS`
- `COMPANION CARE - SUPPLEMENTS` / `NEFROTEC`
- `COMPANION CARE - SUPPLEMENTS` / `REGURIN`
...
**Note.** This category has a hand-audited bespoke script, `categories/pet_care.py`, which is more accurate than these rules alone. Use this prompt for the LLM route; use the script for the deterministic route.
