# Prompt — 1DS "household supplies" -> HGML
...
You receive marketplace product listings from the 1DS category **household supplies**.
...
- `PHARMA - OTHERS` / `HAND SANITIZER`
- `PHARMA - OTHERS` / `QUISTA`
...
1. **Himalaya product name.** If the title names a Himalaya product, use the
   ...
   - "ayurveda secrets clear skin soap" -> BAR SOAPS
   - "ayurveda secrets sandal glow soap" -> BAR SOAPS
   - "bar soaps" -> BAR SOAPS
   - "body butter" -> MOTHER CARE
   - "botanique lavender soap" -> BAR SOAPS
   - "cream honey nourishing soap" -> BAR SOAPS

2. **1DS sub-category.** This field is a product attribute, not seller copy, so
   it is more reliable than any keyword in the title:
   - `hand sanitisers` -> PERSONAL HYGIENE / HAND SANITIZER
