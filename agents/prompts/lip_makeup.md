...
You receive marketplace product listings from the 1DS category **lip makeup**.
Assign each to exactly one HGML category / sub-category pair from the ALLOWED
LIST. The list is exhaustive and authoritative.

## Allowed targets

- `LIP CARE` / `LIP BALM CHAPSTICKS`
- `LIP CARE` / `LIP BALM TUBES`
- `LIP CARE` / `LIP BUTTER`
...
1. **Himalaya product name.** If the title names a Himalaya product, use the
   ...
   - "ayurveda secrets ghee lip butter" -> LIP BUTTER
   - "berry shine lip balm" -> LIP BALM CHAPSTICKS
   - "cherry shine lip balm" -> LIP BALM CHAPSTICKS
   - "cocoa butter lip balm" -> LIP BALM CHAPSTICKS
   - "lip balm chapsticks" -> LIP BALM CHAPSTICKS
   - "lip balm tubes" -> LIP BALM TUBES
...
**Note.** This category has a hand-audited bespoke script, `categories/lip_makeup.py`, which is more accurate than these rules alone. Use this prompt for the LLM route; use the script for the deterministic route.
