...
You receive marketplace product listings from the 1DS category **skin care**.
Assign each to exactly one HGML category / sub-category pair from the ALLOWED
LIST. The list is exhaustive and authoritative.

## Allowed targets

- `BODY MOISTURISERS` / `BODY GELS`
- `BODY MOISTURISERS` / `BODY LOTIONS`
- `BODY MOISTURISERS` / `GP CREAMS`
- `BODY MOISTURISERS` / `HAND CARE`
- `FACE CLEANSERS EXCL. FACE WASH` / `FACE MASKS`
- `FACE CLEANSERS EXCL. FACE WASH` / `FACE SCRUBS`
- `FACE CLEANSERS EXCL. FACE WASH` / `FACIAL WIPES`
- `FACE CLEANSERS EXCL. FACE WASH` / `TONER / MILK`
- `OTX - OTHERS` / `ADULT DIAPERS`
- `OTX - OTHERS` / `ANTISEPTIC CREAM`
- `OTX - OTHERS` / `BALMS`
- `OTX - OTHERS` / `CHYAVANPRASHA`
- `OTX - OTHERS` / `FOOT CARE`
- `OTX - OTHERS` / `MASSAGE OIL`
- `OTX - OTHERS` / `MUSCLE & JOINT RUB`
- `PERSONAL HYGIENE` / `BAR SOAPS`
- `PERSONAL HYGIENE` / `HAND SANITIZER`
- `PERSONAL HYGIENE` / `HAND WASH`
- `PERSONAL HYGIENE` / `HAND WIPES`
- `PERSONAL HYGIENE` / `MOTHER CARE`
- `SUN CARE` / `SUNSCREEN CREAM`
- `SUN CARE` / `SUNSCREEN FLUID`
...
1. **Himalaya product name.** If the title names a Himalaya product, use the
   ...
   - "adult diapers pants" -> ADULT DIAPERS
   - "age defying hand cream" -> HAND CARE
   - "aloe cucumber refreshing body lotion" -> BODY LOTIONS
   - "aloe gel" -> BODY GELS
