-- 009_fix_blinkit_pack_size_parsing.sql
--
-- Fixes pack_size / uom parsing in staging.usp_load_blinkit_products.
--
-- THE BUG
...
-- scoring meaningless for the row. The same defect is present on every channel;
-- this is the Blinkit copy of the amazon fix (008).
--
-- THE FIX
...
-- Measured on staging.blinkit_products; see the commit message for counts.
...
ALTER PROCEDURE [staging].[usp_load_blinkit_products]
AS
BEGIN
    SET NOCOUNT ON;
    SET XACT_ABORT ON;
    ...
    BEGIN TRY
        ...
        INSERT INTO [staging].[blinkit_products]
        (
        ...
        WHERE p.channel_name = 'Blinkit'
          AND sync_status = 'PENDING';

          UPDATE [raw].[oneds_products]
          SET sync_status = 'COMPLETED'
          WHERE channel_name = 'Blinkit' AND sync_status = 'PENDING';

        COMMIT TRANSACTION;

        PRINT 'Blinkit products loaded successfully.';
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0
