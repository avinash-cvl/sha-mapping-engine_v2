-- 011_fix_zepto_pack_size_parsing.sql
--
-- Fixes pack_size / uom parsing in staging.usp_load_zepto_products.
--
-- THE BUG
...
-- this is the Zepto copy of the amazon fix (008).
--
-- THE FIX
...
ALTER PROCEDURE [staging].[usp_load_zepto_products]
AS
BEGIN
    SET NOCOUNT ON;
    SET XACT_ABORT ON;
    ...
    BEGIN TRY
        ...
        INSERT INTO [staging].[zepto_products]
        (
        ...
        WHERE p.channel_name = 'Zepto'
          AND sync_status = 'PENDING';

          UPDATE [raw].[oneds_products]
          SET sync_status = 'COMPLETED'
          WHERE channel_name = 'Zepto' AND sync_status = 'PENDING';

        COMMIT TRANSACTION;

        PRINT 'Zepto products loaded successfully.';
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0
