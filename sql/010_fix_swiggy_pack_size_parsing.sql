-- 010_fix_swiggy_pack_size_parsing.sql
--
-- Fixes pack_size / uom parsing in staging.usp_load_swiggy_products.
--
-- THE BUG
...
-- this is the Swiggy copy of the amazon fix (008).
--
-- THE FIX
...
-- Measured on staging.swiggy_products; see the commit message for counts.
...
ALTER PROCEDURE [staging].[usp_load_swiggy_products]
AS
BEGIN
    SET NOCOUNT ON;
    SET XACT_ABORT ON;
    ...
    BEGIN TRY
        ...
        INSERT INTO [staging].[swiggy_products]
        (
        ...
        WHERE p.channel_name = 'Swiggy'
          AND sync_status = 'PENDING';

          UPDATE [raw].[oneds_products]
          SET sync_status = 'COMPLETED'
          WHERE channel_name = 'Swiggy' AND sync_status = 'PENDING';

        COMMIT TRANSACTION;

        PRINT 'Swiggy products loaded successfully.';
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0
